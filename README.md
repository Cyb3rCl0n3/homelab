[![OpenSSF Scorecard](https://api.securityscorecards.dev/projects/github.com/Cyb3rCl0n3/homelab/badge)](https://securityscorecards.dev/viewer/?uri=github.com/Cyb3rCl0n3/homelab)
...
|Event|Path|Jobs|
|---|---|---|
|Pull request|any|`terraform-scan`, `ansible-scan` `terraform-plan`|
|Push to main|`proxmox/terraform/**`, `proxmox/ansible/**`, `.github/workflows/**`| `terraform-scan`, `ansible-scan`,`terraform-apply`, `ansible-run`

### Diagram
```mermaid
flowchart LR

  subgraph SC["Code Scan"]
    subgraph TS["Terraform Scan"]
      direction TB
      TSB(Validate) --> TSC(Tflint) --> TSD(Trivy) --> TSE(Checkov) --> TSF(Upload SARIF) --> TSN(Gate on findings)
    end
    subgraph AS["Ansible Scan"]
      direction TB
      ASA(Ansible-lint) --> ASB(Checkov) --> ASC(Upload SARIF) --> ASN(Gate on findings)
    end
  end
